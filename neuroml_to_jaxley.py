...
try:
    from jaxley_mech.synapses import GapJunction  # optional
except Exception:
    GapJunction = None
...
def parse_neuroml_to_intermediate(nml_doc: neuroml.NeuroMLDocument) -> NetworkSpec:
    ...
    for pop in getattr(net, "populations", []) or []:
        ...
        # component defaults: if the referenced component exists (e.g., iafCell), flatten key params
        comp_obj = cell_index.get(comp)
...
def default_cell_mapper(c: CellSpec) -> JaxleyCellParams:
    ...
    gNa = _pick(["cell.gNa", "cell.gNa_S_per_cm2", "pop.gNa", "gNa_S_per_cm2", "gNa"])
    gK  = _pick(["cell.gK",  "cell.gK_S_per_cm2",  "pop.gK",  "gK_S_per_cm2",  "gK"])
    gL  = _pick(["cell.gL",  "cell.gLeak",        "cell.gLeak_S_per_cm2",
                 "pop.gL",   "pop.gLeak",         "gLeak_S_per_cm2", "gL", "gLeak"])
    ...
    eNa = _pick(["cell.E_Na_mV", "cell.ENa_mV", "cell.eNa_mV", "pop.E_Na_mV", "pop.ENa_mV",
                 "cell.E_Na_V",  "cell.ENa_V",  "cell.eNa",   "pop.eNa",     "ENa", "E_Na", "eNa"])
    eK  = _pick(["cell.E_K_mV",  "cell.EK_mV",  "cell.eK_mV",  "pop.E_K_mV",  "pop.EK_mV",
                 "cell.E_K_V",   "cell.EK_V",   "cell.eK",    "pop.eK",      "EK",  "E_K",  "eK"])
...
def default_conn_mapper(e: ConnSpec) -> Optional[JaxleyConnParams]:
    ...
    if e.kind == "gap":
        ...
        return JaxleyConnParams(kind="gap", syn_class="GapJunction",
                                syn_kwargs={"gGap": float(e.params["g_gap_S"])})
    return None
...
# ---- Connections (robust) ----
    ...
    for edge in spec.conns:
        ...
        # Set parameters, tolerating schema variants
        if jp.kind == "chem":
            ...
            # Optional kinetics if provided
            if "tau_rise_ms" in jp.syn_kwargs:
                for name in ("IonotropicSynapse_tau_rise_ms", "tau_rise_ms", "tauRise_ms"):
                    try:
                        net.IonotropicSynapse.edge(eid).set(name, float(jp.syn_kwargs["tau_rise_ms"]))
                        break
                    except Exception:
                        continue
            if "tau_decay_ms" in jp.syn_kwargs:
                for name in ("IonotropicSynapse_tau_decay_ms", "tau_decay_ms", "tauDecay_ms"):
                    try:
                        net.IonotropicSynapse.edge(eid).set(name, float(jp.syn_kwargs["tau_decay_ms"]))
                        break
                    except Exception:
                        continue
        else:  # gap
...
# -----------------------------
def pulses_to_currents(inputs_flat: List[Dict[str, Any]], *, dt_ms: float, t_max_ms: float, amp_scale: float = 1.0) -> Dict[str, np.ndarray]:
    """Convert flattened inputs (delay_ms, duration_ms, amplitude_A) into per-pop current traces.
    Returns a dict: population_id -> np.ndarray [timepoints] of absolute current in A.
    """
    ...
    for ent in inputs_flat or []:
        ...
        amp_A = (ent.get("amplitude_A") or 0.0) * float(amp_scale)
        ...
        if i1 > i0:
            currents[pop][i0:i1] += float(amp_A)
    return currents
...
def load_c302_preset_to_intermediate(preset: str) -> NetworkSpec:
    ...
    if preset.upper() in ("IA", "IB"):
        ...
        level = preset[-1]
        out = c302_IClamp.setup(level, generate=True, verbose=False)

    elif preset.upper() in ("C", "FULL"):
        from c302 import c302_Full
        level = "A" if preset.upper() == "FULL" else "C"
        out = c302_Full.setup(level, generate=True, verbose=False, muscles_to_include=False)
    ...
    # Case 2: subsystem presets like "Pharyngeal:B"
        ...
        out = mod.setup(level.strip(), generate=True, verbose=False)
...
# Make net.cell accept Python ints by wrapping them as np.array([idx])
    ...
    #     color="k", synapse_color="r",
    #     synapse_scatter_kwargs={"s": 10, "alpha": 0.7}
    # )
    ...
    if inputs_flat:
        ...
        # Example: scale pA by 1e3 to nA, or by membrane area if you want current density.
        currents = pulses_to_currents(inputs_flat, dt_ms=args.dt, t_max_ms=args.duration, amp_scale=1e9)
    ...
    record_indices = []
    seen_idx = set()
    for i in presyn_indices:
        if i not in seen_idx:
            seen_idx.add(i); record_indices.append(i)
    for i in postsyn_indices:
        if i not in seen_idx:
            seen_idx.add(i); record_indices.append(i)
